...
class CollectionOpsTest extends AnyWordSpec with Matchers {
  "IndexedSeqOps" should {
    ...
    "split index seq in 3" in {
      ...
      left must contain theSameElementsInOrderAs (Vector(1, 2))
      mid must contain theSameElementsInOrderAs (Vector(3, 4))
      right must contain theSameElementsInOrderAs (Vector(5, 6, 7))
    }

    "return empty left when point1 is 0" in {
      val (left, mid, right) = Vector(1, 2, 3, 4, 5, 6, 7).splitInThree(0, 7)
      left must contain theSameElementsInOrderAs (Vector())
      mid must contain theSameElementsInOrderAs (Vector(1, 2, 3, 4, 5, 6))
      right must contain theSameElementsInOrderAs (Vector(7))
    }
