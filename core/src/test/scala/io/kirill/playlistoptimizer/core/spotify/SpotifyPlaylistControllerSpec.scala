...
class SpotifyPlaylistControllerSpec extends ControllerSpec {
  ...
  val optimization           = Optimization(optimizationId, BigDecimal(0), optimizationParameters, shortenedPlaylist, timestamp)
  val jwt =
  ...
  val sessionCookie = RequestCookie("spotify-session", jwt)
  val accessToken   = SpotifyAccessToken("access-token", "refresh-token", "user-id", timestamp)
  ...
  "A SpotifyPlaylistController" should {
    ...
    "return existing user-session cookie" in {
      ...
      val spotifyAccessToken = SpotifyAccessToken("access-token", "refresh-token", "user-id", timestamp)
    ...
    "GET /tracks - find track by name" in {
      ...
      val expected = TrackView(
        "Glue",
        List("Bicep"),
        PlaylistBuilder.defaultRelease,
        None,
        129.983,
        269.15,
        5,
        ...
        Some("https://open.spotify.com/track/2aJDlirz6v2a4HREki98cP")
      )
    ...
    "GET /tracks - return bad request when query param is missing" in {
      ...
      verifyResponse[ErrorResponse](
        response,
        Status.BadRequest,
        Some(ErrorResponse("query parameter name is required to make this request"))
      )
      verifyNoInteractions(jwtEncoder, service)
    }
    ...
    "POST /playlists/import - import new playlist into spotify" in {
      ...
      val controller            = new SpotifyPlaylistController(jwtEncoder, service, sc)
      when(jwtEncoder.decode(sessionCookie.content)).thenReturn(IO.pure(accessToken))
      when(service.findTracksByNames(eqTo(accessToken), anyList[String]))
        .thenReturn(IO.pure((shortenedPlaylist.tracks.toList, accessToken)))
      ...
      verify(service).save(
        accessToken,
        Playlist("New Imported playlist", Some("Imported playlist with songs"), shortenedPlaylist.tracks, PlaylistSource.Spotify)
      )
    }
  }
